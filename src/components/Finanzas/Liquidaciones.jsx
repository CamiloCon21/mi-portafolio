...
function Liquidaciones() {
  ...
  const calcular = (e) => {
    ...
    const diferenciaDias = Math.floor((fechaFin - fechaInicio) / msPorDia) + 1;
    ...
    const vacaciones = ((salario * diferenciaDias) / 720) + (salarioDiario * diasAcumulados);
      ...
      {resultado && (
        <div className="mt-6 bg-white p-6 rounded-lg shadow-md max-w-lg mx-auto">
          ...
          <ul className="space-y-2">
            ...
            <li><strong>Cesantías:</strong> ${resultado.cesantias.toFixed(2)}</li>
            <li><strong>Intereses sobre cesantías:</strong> ${resultado.interesesCesantias.toFixed(2)}</li>
            <li><strong>Prima:</strong> ${resultado.prima.toFixed(2)}</li>
            <li><strong>Vacaciones:</strong> ${resultado.vacaciones.toFixed(2)}</li>
            {resultado.subsidioTransporte > 0 && (
              <li><strong>Subsidio de transporte:</strong> ${resultado.subsidioTransporte.toFixed(2)}</li>
            )}
            <li className="text-lg font-bold"><strong>Total Liquidación:</strong> ${resultado.total.toFixed(2)}</li>
          </ul>
